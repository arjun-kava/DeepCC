...
% load(fullfile('D:\Workspace\Research\Learn\DeepCC-master\experiments\demo\L3-identities', 'L2trajectories_test_easy.mat'));
% trajectories = trainData;
filename = sprintf('%s/%s/L3-identities/L3-identities_%s.mat',opts.experiment_root, opts.experiment_name, opts.sequence_names{opts.sequence});
load(filename);
% Load map
...
endFrame   = interval(end);%interval(1) + fps * 100;%interval(end);
